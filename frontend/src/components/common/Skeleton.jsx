  ...
  <tr className="border-b border-slate-100 animate-pulse">
    {Array.from({ length: columns }).map((_, i) => (
      <td key={i} className="py-3.5 px-4">
        <div className="h-4 bg-slate-200/70 rounded w-full max-w-[120px]" />
      </td>
    ))}
  </tr>
);
...
export const CardSkeleton = () => (
  <div className="bg-white border border-slate-200 rounded-lg p-5 animate-pulse flex flex-col gap-3">
...
export const MetricSkeleton = () => (
  <div className="bg-white border border-slate-200 rounded-lg p-5 animate-pulse">
    ...
    <div className="h-7 bg-slate-300 rounded w-2/3 mb-2" />
    <div className="h-3 bg-slate-100 rounded w-1/2" />
  </div>
);
