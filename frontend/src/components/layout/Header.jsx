...
export const Header = () => {
  ...
  return (
    <div className="sticky top-0 z-50 w-full bg-white dark:bg-[#0B0F19] border-b border-slate-200 dark:border-slate-800 shadow-xs transition-colors duration-200">
      ...
      <header className="w-full bg-white dark:bg-[#0B0F19] border-b border-slate-100 dark:border-slate-800/80 px-4 sm:px-6 lg:px-8 py-2.5 sm:py-3 flex items-center justify-between gap-3 sm:gap-6">
        ...
        <div className="flex items-center gap-1.5 sm:gap-3 shrink-0">
          ...
          <button
            ...
            className="md:hidden p-2 text-slate-600 hover:text-orange-600 hover:bg-orange-50/60 rounded-xl transition-colors cursor-pointer"
            title="Qidiruv"
          >
            <SolarIcon name="Search" size={19} />
          </button>

          {/* Dark / Light Theme Toggle */}
          <button
            onClick={toggleTheme}
            className="p-2 text-slate-500 hover:text-amber-500 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-xl transition-all duration-200 cursor-pointer flex items-center justify-center relative group"
          ...
          >
            {isDark ? (
              ...
              <SolarIcon name="moon" size={20} className="text-slate-600 group-hover:-rotate-12 transition-transform duration-300" />
            )}
          </button>
          ...
          {isAuthenticated && (
            <div className="relative" ref={notifRef}>
              <button
                ...
                className="p-2 text-slate-500 hover:text-orange-600 hover:bg-orange-50/60 rounded-xl transition-colors relative cursor-pointer"
              ...
              {showNotifs && (
                <div className="absolute right-0 mt-2 w-80 sm:w-96 bg-white border border-slate-200 rounded-2xl shadow-2xl py-2 z-50 animate-in fade-in slide-in-from-top-2">
                    ...
                    {notifications.length === 0 ? (
                      ...
                      notifications.map(n => (
                        <div
                          ...
                          className={`p-3 text-left hover:bg-slate-50 cursor-pointer transition-colors ${!n.is_read ? 'bg-orange-50/40' : ''}`}
                        >
          ...
          {isAuthenticated ? (
            <div className="relative" ref={userMenuRef}>
              ...
              {/* User Dropdown Menu */}
              {showUserMenu && (
                <div className="absolute right-0 mt-2 w-64 bg-white border border-slate-200 rounded-2xl shadow-2xl py-2 z-50 animate-in fade-in slide-in-from-top-2">
                  <div className="px-4 py-3 border-b border-slate-100">
                    ...
                    <div className="mt-1.5 inline-block text-[10px] uppercase font-bold bg-orange-100 text-orange-800 px-2 py-0.5 rounded">
                      Rol: {user?.role}
                    </div>
                  </div>
                      ...
                      <div className="my-1 pt-1 border-t border-slate-100">
                        ...
                        {isSeller && (
                          <button
                            ...
                            className="w-full flex items-center gap-2.5 px-4 py-2 text-xs text-orange-700 hover:bg-orange-50 text-left font-semibold transition-colors cursor-pointer"
                          >
                        ...
                        {isAdmin && (
                          <button
                            ...
                            className="w-full flex items-center gap-2.5 px-4 py-2 text-xs text-red-700 hover:bg-red-50 text-left font-semibold transition-colors cursor-pointer"
                          >
                  ...
                  <div className="py-2 px-4 border-t border-slate-100 flex items-center justify-between">
                    ...
                    <button
                      type="button"
                      onClick={toggleTheme}
                      className={`w-9 h-5 flex items-center rounded-full p-0.5 cursor-pointer transition-colors duration-300 ${isDark ? 'bg-orange-600 justify-end' : 'bg-slate-300 justify-start'}`}
                  ...
                  <div className="pt-1 border-t border-slate-100">
                    <button
                      ...
                      className="w-full flex items-center gap-2.5 px-4 py-2 text-xs text-rose-600 hover:bg-rose-50 text-left font-semibold transition-colors cursor-pointer"
                    >
            ...
            <div className="flex items-center gap-2">
              <button
                ...
                className="text-xs font-semibold text-slate-700 hover:text-orange-600 px-3 py-2 rounded-xl transition-colors cursor-pointer"
              >
                Kirish
              </button>
              <button
                onClick={() => navigate('/register')}
                className="text-xs font-bold text-white bg-orange-600 hover:bg-orange-700 px-3.5 sm:px-4 py-2 rounded-xl shadow-sm transition-all active:scale-95 cursor-pointer whitespace-nowrap"
              >
                Ro'yxatdan o'tish
              </button>
            </div>
      ...
      {/* Mobile Expandable Search Bar with Autocomplete */}
      {mobileSearchOpen && (
        <div className="md:hidden px-3.5 py-2.5 bg-white dark:bg-[#0B0F19] border-b border-slate-200 dark:border-slate-800 animate-in fade-in slide-in-from-top-1 shadow-md">
          <SearchAutocomplete
            value={searchQuery}
            onChange={setSearchQuery}
            onSubmit={(term) => {
