...
const GoogleLoginButton = ({
  ...
  const clientId = import.meta.env.VITE_GOOGLE_CLIENT_ID || "355205646431-6c3sa3um31o322ffr8800o58ip3gsl5h.apps.googleusercontent.com";
  ...
  useEffect(() => {
    ...
    const setupGoogle = () => {
      if (!window.google?.accounts?.id || !buttonRef.current || !containerRef.current) return false;
      ...
      try {
        ...
        // Determine optimal width and theme
        const isDark = document.documentElement.classList.contains('dark');
        const containerWidth = containerRef.current.offsetWidth || 380;
        // Google requires width between 200 and 400
        const buttonWidth = Math.min(Math.max(containerWidth, 220), 400);
    ...
    // Observe Dark Mode changes to instantly re-theme the button
    const observer = new MutationObserver(() => {
      setupGoogle();
    });
    observer.observe(document.documentElement, { attributes: true, attributeFilter: ['class'] });

    // Handle window resize so button adjusts to container width
    const handleResize = () => {
      setupGoogle();
    };
    ...
    <div ref={containerRef} className={`w-full ${className}`}>
      {loading ? (
        ...
        <div className="w-full flex flex-col items-center">
          {/* Skeleton while Google script loads, then smoothly replaced */}
          {!isReady && (
            <div className="w-full py-3 px-5 bg-white dark:bg-[#151D2C] border border-slate-200 dark:border-slate-800 text-slate-700 dark:text-slate-300 font-bold rounded-2xl text-xs sm:text-sm flex items-center justify-center gap-2.5 shadow-xs animate-pulse">
          ...
          {/* Official Google Button rendered with native dimensions and zero clipping */}
          <div
            ref={buttonRef}
